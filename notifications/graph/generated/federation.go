...
func (ec *executionContext) __resolve_entities(ctx context.Context, representations []map[string]interface{}) []fedruntime.Entity {
	...
	resolveEntity := func(ctx context.Context, typeName string, rep map[string]interface{}, idx []int, i int) (err error) {
		...
		switch typeName {
		case "Notification":
			resolverName, err := entityResolverNameForNotification(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "Notification": %w`, err)
			}
			switch resolverName {
			...
			case "findNotificationByID":
				...
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findNotificationByID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindNotificationByID(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "Notification": %w`, err)
				}
...
func entityResolverNameForNotification(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
		...
		return "findNotificationByID", nil
	}
	return "", fmt.Errorf("%w for Notification", ErrTypeNotFound)
}
